...
(define racketlist->mupllist
  (lambda(racketlist) (cond [(null? racketlist) (aunit)]
                            [#t (apair (car racketlist) (racketlist->mupllist (cdr racketlist)))])))

(define mupllist->racketlist
  (lambda(mupllist) (cond [(aunit? mupllist) null]
  ...
  (cond [(var? e) 
        ...
        [(call? e) 
         (letrec ([v1 (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? v1)
               (letrec ([nameopt (cons (fun-nameopt (closure-fun v1)) v1)]
                        [formal (cons (fun-formal (closure-fun v1)) v2)])
                 (eval-under-env
                  (fun-body (closure-fun v1))
                  (if (eq? (car nameopt) #f)
                      (cons formal (closure-env v1))
                      (cons formal (cons nameopt (closure-env v1))))))
...
(define mupl-map 
  (fun "function1" "function2" 
       (fun "function3" "lst" 
            (ifaunit (var "lst")
                    (aunit)
                    (apair
                     (call (var "function2") (fst (var "lst")))
                     (call (var "function3") (snd (var "lst"))))))))
